...
export const useGameLoop = ({ gameStarted, gameOver, setGameOver, setScore }: GameLoopProps) => {
  ...
  const checkCollision = useCallback(() => {
    ...
    if (dragonTop <= 0 || dragonBottom >= gameDimensions.height) {
      return true;
    }
  ...
  const gameLoop = useCallback(() => {
    ...
    setDragonPos(prev => prev + dragonVelocity);
    setDragonVelocity(prev => prev + gravity);
    
    // Update dragon rotation based on velocity for visual feedback
    setDragonRotation(Math.min(Math.max(dragonVelocity * 3, -30), 90));
    ...
    requestRef.current = requestAnimationFrame(gameLoop);
  }, [gameStarted, gameOver, dragonVelocity, gravity, checkCollision, updateScore, setGameOver]);
